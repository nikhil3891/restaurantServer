...
## 31 May 2026 — Testing Infrastructure + API Testing Guide

### What We Did

#### 1. Fixed Stale Unit Tests
- **Files:** `src/app.controller.spec.ts`, `test/app.e2e-spec.ts`
- **Why:** Both files were generated by NestJS CLI and still tested `Hello World!` which no longer exists. Running `pnpm test` would fail immediately.
- **What changed:**
  - `app.controller.spec.ts` — updated to test the actual `root()` and `health()` methods
  - `test/app.e2e-spec.ts` — updated to test `/api/v1/health` endpoint and check that auth routes return non-404
- **Result:** All 12 unit tests passing

#### 2. Wrote AuthService Unit Tests
- **File:** `src/modules/auth/services/auth.service.spec.ts`
- **Why:** AuthService is the most critical service in the app. Having tests for it ensures register, login, refresh, logout, and validateUser all behave correctly before we add more modules on top.
- **Pattern used:** Mock all dependencies (`PrismaService`, `UsersService`, `JwtService`, `ConfigService`) using `jest.fn()` — no real database needed
- **10 test cases cover:**
  - `validateUser` — correct password, wrong password, user not found, inactive user
  - `register` — creates user and returns tokens
  - `login` — returns tokens for valid user
  - `refreshTokens` — valid token, non-existent token, expired token
  - `logout` — revokes all tokens

#### 3. Created API Testing Guide
- **File:** `docs/api/API-TESTING-GUIDE.md`
- **Why:** As the project grows to 14+ modules and 100+ endpoints, developers need a reference that explains:
  - Which tools to use (Swagger, cURL, Thunder Client)
  - How to authenticate in Swagger UI
  - Exactly what request body each endpoint expects
  - What every possible error response looks like
  - How to test edge cases (wrong role, expired token, empty body)
- **Covers:**
  - Swagger UI setup with bearer auth
  - cURL commands for every auth endpoint
  - Thunder Client environment + auto-save token setup
  - Full request/response docs for all 5 auth endpoints
  - A copy-paste template for documenting future module endpoints
  - Common error scenarios table (test these for every new endpoint)
  - Quick reference table of all current endpoints

#### 4. Updated All Documentation Index Files
- **`docs/README.md`** — Added full quick links table covering all 12 key documents (was only 2 links before)
- **`docs/api/README.md`** — Added link to API-TESTING-GUIDE.md
- **`docs/guides/README.md`** — Kept clean, no changes needed
...
- **Pre-coding checklist status:**
  - [ ] ER diagram for `restaurants` and `branches` tables — see `docs/database/ER-DESIGN-GUIDE.md`
  - [ ] RBAC design (SuperAdmin creates restaurants, Owner manages own)
  - [ ] Multi-tenant column strategy confirmed
  - [ ] API contract defined — add to `docs/api/API-TESTING-GUIDE.md` when done
